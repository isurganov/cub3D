...
/*   Updated: 2022/09/25 14:31:02 by ilya             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void side_dist(t_game *game)
{
	...
	{
		game->ray.step_x = -1;
		game->ray.sd.x = (game->pl.pos.x - game->map.x_pl) * game->ray.dd.x;
	}
	else
	{
		game->ray.step_x = 1;
		game->ray.sd.x = (game->map.x_pl + 1.0 - game->pl.pos.x) * game->ray.dd.x;
	}
	if (game->ray.ray_dir.y < 0)
	{
		game->ray.step_y = -1;
		game->ray.sd.y = (game->pl.pos.y - game->map.y_pl) * game->ray.dd.y;
	}
	else
	{
		game->ray.step_y = 1;
		game->ray.sd.y = (game->map.y_pl + 1.0 - game->pl.pos.y) * game->ray.dd.y;
	}
}
...
static void DDA_algo(t_game *game)
{
    while (game->ray.hit == 0)
	{
		...
		if (game->map.map[(int)(game->map.y_pl)][(int)(game->map.x_pl)] == '1' ||
			game->map.map[(int)(game->map.y_pl)][(int)(game->map.x_pl)] == '2')
			game->ray.hit = 1;
	}
}
